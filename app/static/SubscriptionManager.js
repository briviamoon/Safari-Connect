...
async function subscribeUserForPlan(selectedPlan) {
  ...
  try {
    ...
    if (response.data && response.data.message) {
      ...
      showSuccess(
        "Subscription initiated. Please complete payment on your phone."
      );
    }
...
function startPaymentStatusPolling() {
  ...
  console.log("Starting Polling on Payment status...");
  paymentStatusInterval = setInterval(
    () => checkUserSubscription(getCurrentUser().user_id),
    2000
  );
}
...
async function checkUserSubscription(userId) {
  ...
  try {
    console.log("checking Spubscription for User: ", userId);
    const response = await axios.get(
      `${API_BASE_URL}/subscription/subscription-status`,
      {
...
function handleSubscriptionResponse(response) {
  ...
  localStorage.setItem("subscriptionActive", activeSub); // Updating the local storage

  unmarkSubscriptionChecked();
  if (activeSub) {
    console.log("Subscription is active, Stopping the Poll now! ...");
    clearInterval(paymentStatusInterval);
...
function unmarkSubscriptionChecked() {
